...
* Can savestate restore be more lightweight? What takes so much CPU time? Are there any lingering spurious disk accesses? Also, I think netplay efficiency may rely on stable data positions, so double check this to see if structures are moving around from frame to frame (I suspect it only really changes at disk insert/eject). In the advanced menu there's a performance profiling option that will list the timings of the last savestate. On my laptop save is about 8ms, and load is about 24ms. Not good enough for real-time, but not impossibly far off either. (Emulating a frame takes about 1.3ms by comparison.)
