...
* Saving:
  ...
  * Note that Hatari is not able to simulate the formatting of a blank disk in-emulator for the ST, MSA, or DIM formats. The [stand-alone version of Hatari](https://hatari.tuxfamily.org/download.html) has a utility in its menu that can be used to create a blank ST file, or other emulators like [Steem.SSE](https://sourceforge.net/projects/steemsse/) can simulate the formatting process in-emulator.
